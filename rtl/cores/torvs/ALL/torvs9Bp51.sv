...
module torv32 (
	...
	wire ba_fd_rs1_HAZ = !b_fd_NOP & b_ins_ALL & reads_rs1(
		b_fd_IR
	) & rs1ID(
		b_fd_IR
	) != 0 & ((writes_rd(
	...
	wire ba_fd_rs2_HAZ = !b_fd_NOP & b_ins_ALL & reads_rs2(
		b_fd_IR
	) & rs2ID(
		b_fd_IR
	) != 0 & ((writes_rd(
	...
	wire [31:0] a_d_JoB_ADDR = a_fd_IR[3:2] == 2'b01 ? RAS_0 : (a_fd_PC + (isJAL(
		a_fd_IR
	) ? Jimm(
		a_fd_IR
	) : Bimm(
	...
	wire [31:0] b_d_JoB_ADDR = b_fd_IR[3:2] == 2'b01 ? RAS_0 : (b_fd_PC + (isJAL(
		b_fd_IR
	) ? Jimm(
		b_fd_IR
	) : Bimm(
	...
	reg [31:0] a_de_IR, a_de_PC, a_de_predictRA;
	reg a_de_predict;
	reg [BHT_ADDR_BITS-1:0] a_de_BHTindex;
	reg [1:0] a_de_BHT_data;
	reg [31:0] b_de_IR, b_de_PC, b_de_predictRA;
	reg b_de_predict;
	...
	*/
	...
	wire a_e_JoB = ((isJALR(
		a_de_IR
	) && (a_de_predictRA != a_e_ADDR)) || (isBtype(
		a_de_IR
	) & (a_e_takeB ^ a_de_predict)));
	...
	*/
	...
	wire b_e_JoB = ((isJALR(
		b_de_IR
	) && (b_de_predictRA != b_e_ADDR)) || (isBtype(
		b_de_IR
	) & (b_e_takeB ^ b_de_predict)));
