...
module torv32 #(
	...
	generate
		if (BYPASS) begin
			...
			assign e_rs1 = e_m_fwd_rs1 ? em_RES : e_w_fwd_rs1 ? wb_DATA : de_rs1;

			assign e_rs2 = e_m_fwd_rs2 ? em_RES : e_w_fwd_rs2 ? wb_DATA : de_rs2;

		end else begin

			assign rs1_HAZ = !fd_NOP & reads_rs1(
				fd_IR
			) & rs1ID(
				fd_IR
			) != 0 & ((writes_rd(
			...
			assign rs2_HAZ = !fd_NOP & reads_rs2(
				fd_IR
			) & rs2ID(
				fd_IR
			) != 0 & ((writes_rd(
	...
	generate
		if (BRANCH_PRED == 4'b0000) begin
			...
			assign f_PC       = d_JoB_now ? d_JoB_ADDR : em_JoB ? em_JoB_ADDR : PC;
			...
			always @(posedge clk) if (!d_stall) de_predict <= d_predict;
			...
			always @(posedge clk) begin
				if (isBtype(de_IR)) BHT[de_BHT_index] <= incdec_sat(BHT[de_BHT_index], e_takeB);
			end
	...
	assign wb_DATA = isLoad(
		mw_IR
	) ? (W_isIO ? mw_IO_RES : w_mem_RES) : isCSRRS(
		mw_IR
	) ? mw_CSR_RES : mw_RES;
	...
	function isEBREAK;
		input [31:0] I;
		isEBREAK = ((I[6:0] == 7'b1110011) && I[14:12] == 3'b000);
	endfunction
	function isCSRRS;
		input [31:0] I;
		isCSRRS = ((I[6:0] == 7'b1110011) && I[14:12] == 3'b010);
	endfunction
	...
	always @(posedge clk) begin
		if (halt) begin
			...
			$display("Instr. mix = (Branch:%3.3f\%% JAL:%3.3f\%% JALR:%3.3f\%%)",
					 nbBranch * 100.0 / instret, nbJAL * 100.0 / instret, nbJALR * 100.0 / instret);
