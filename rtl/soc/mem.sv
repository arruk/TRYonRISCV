...
`else
	...
	initial begin
		if (!$value$plusargs("PROGROM=%s", progrom_path))
			$fatal(1, "ERRO: informe +PROGROM=<arquivo>");

		if (!$value$plusargs("DATARAM=%s", dataram_path))
			$fatal(1, "ERRO: informe +DATARAM=<arquivo>");


		$readmemh(progrom_path, ROM);
		$readmemh(dataram_path, RAM);
	end
`endif
...
`endif
	...
	always @(posedge clk) begin
		if (imem_en) imem_data <= ROM[imem_addr_seg];
	end
endmodule
...
`endif
	...
	always @(posedge clk) begin
		if (a_imem_en) a_imem_data <= ROM[a_imem_addr_seg];
	end

	always @(posedge clk) begin
		if (b_imem_en) b_imem_data <= ROM[b_imem_addr_seg];
	end
...
module mem (
	...
	always @(posedge clk) begin
		if (imem_en) imem_data <= RAM[imem_addr_seg];
	end
endmodule
