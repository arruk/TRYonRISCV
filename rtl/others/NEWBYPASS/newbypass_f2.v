...
module torv32 (
	...
	wire [31:0] e_rs1 = e_m_fwd_rs1 ? em_RES : de_rs1;
	wire [31:0] e_rs2 = e_m_fwd_rs2 ? em_RES : de_rs2;
	...
	always @(posedge clk) begin
		...
		em_RES <= e_RES;
		em_ADDR <= e_ADDR;
		em_queue <= e_data_HAZ ? 1'b1 : !(writes_rd1(de_IR) & (rdID(de_IR) != 0)) ? 1'b0 : em_queue;
		em_JoB_now <= e_JoB;
	...
	assign wb_DATA = isLoad(
		em_IR
	) ? (em_ADDR[22] ? IO_mem_rdata : e_mem_RES) : isCSRRS(
		em_IR
	) ? em_CSR_RES : em_queue ? em_RES : e_RES;

	assign wb_enable = ((isLoad(
		em_IR
	) | isCSRRS(
		em_IR
	)) & (rdID(
	...
	function writes_rd1;
		input [31:0] I;
		writes_rd1 = !isStype(I) & !isBtype(I) & !isLoad(I) & !isCSRRS(I);
	endfunction
	function reads_rs1;
		input [31:0] I;
